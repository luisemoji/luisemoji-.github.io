...
// Initial states...  black es .set(game, { userSelect: 'none', background: 'radial-gradient(#333, #000 120%)' })
new TimelineMax({ onStart: populate })
    ...
    .set('.foundTxt', { left: 0, top: 30, width: 1024, fontSize: 35, textAlign: 'center', textContent: 'Encuentra el par correspondiente...', fontWeight: 300, letterSpacing: 0.25 })
...
function populate() {
    ...
    matched = false;
    msTilHint = 5000;
    TweenMax.set(hintBtn, { autoAlpha: 0, textContent: "¿Ayuda?", fontSize: 35 });
    ...
    if (lastBtn != undefined && lastBtn != b) {
        if (b.textContent == lastBtn.textContent) { //matched
            ...
            new TimelineMax({ onComplete: populate })
                .set('.foundTxt', { textContent: 'Encontrados: ' + found, fontWeight: 500 }, 0)
...
function updateTime() {
    if (timeLeft > 0) {
        ...
        TweenMax.set('.endTxt', { textContent: hint + " ¿Intentar de nuevo? " + hint, fontSize: 35 });
...
replayBtn.onclick = replayBtn.ontouchend = function() {
    new TimelineMax()
        ...
        .call(function() { // Reset game...
            TweenMax.set('.foundTxt', { textContent: 'Encontrados: 0' });
